...
import "./Error.sol";
import "./interfaces/IWEFairFactory.sol";
...
contract WEFair is Ownable, EIP712 {
    ...
    function invest(uint256 investAmount) external payable lock {
        ...
        if (_isNative()) {
            ...
            address wefair = address(this);
            if (_investToken.allowance(_msgSender(), wefair) < investAmount) {
                revert InsufficientAllowedInvestAmount();
            }
            if (
                !_investToken.transferFrom(_msgSender(), wefair, investAmount)
            ) {
    ...
    function transferLiquidity(
        ...
        IWEFairFactory _factory = IWEFairFactory(factory);
        ...
        uint256 lockPresaleAmount = parameters.totalSellAmount;
    ...
    function getCanClaimTotal() public view returns (uint256, uint256) {
        ...
        uint256 canClaimTotal = parameters.totalSellAmount
            .mul(_balance)
            .div(totalInvest)
            .sub(claimed[_msgSender()]);

        return (canClaimTotal, canClaimTotal);
    }
    ...
    function getAmount(
        ...
        if (presaleAmount > 0) {
            investAmount = presaleAmount
                .mul(totalInvest)
                .div(parameters.totalSellAmount);
        } else {
            presaleAmount = investAmount
                .mul(parameters.totalSellAmount)
                .div(totalInvest);
        }
